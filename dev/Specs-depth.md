...
## Plan de travail pour la prochaine session

### Étape 1 — Fixture
Créer `tests/fixtures/depth-move/` avec `eventer.db` correspondant exactement à la structure ci-dessus (état initial).

Fichier builder : `tests/helpers/rebuild_fixture_depth_move.rb`

### Étape 2 — Tests (TDD)

Fichier spec : `tests/specs/` (à préciser selon convention du projet — ruby ou playwright)

Tester dans cet ordre :
1. **Vérifier l'état initial** : charger le projet, vérifier que les depths calculés sont 1/2/2/3 pour les listes #2/#3/#5/#4.
2. **Cas 1** : déplacer e45 de Liste #3 vers Liste #5 → vérifier depths inchangés.
3. **Cas 2** : déplacer e31 de Liste #3 vers Liste #5 → vérifier Liste #4 reste à depth=3.
4. **Cas 3** : déplacer e31 de Liste #3 vers Liste #2 → vérifier Liste #4 passe à depth=2.

### Étape 3 — Implémentation

Après RED confirmé sur chaque cas :
- Backend : route PATCH qui met à jour les deux item_ids (source et destination)
- Frontend : après move, relancer la traversée top-down et mettre à jour les depths en mémoire

---

## Note sur le schéma DB

La colonne `depth` existe déjà dans `listers` (database.rb ligne 12). Elle peut rester en DB comme cache optionnel, mais **l'autorité est la traversée**, pas la valeur stockée.

Non, Claude, je n’aime pas ce genre de confusion. Il faut retirer cette colonne de la base de données, dans la table `listers`. Et d’ailleurs, pour éviter toute confusion, je la retire dès à présent. Penser à la gérer dans les fixtures existantes.
