...
- Name: _tdd/split-pane.spec.js >> popup contient les options Vertical et Horizontal
- Location: specs/e2e/_tdd/split-pane.spec.js:40:1
...
Locator: locator('#pane-1').contentFrame().locator('.popup-select').locator('.popup-select__option').filter({ hasText: 'Vertical' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('#pane-1').contentFrame().locator('.popup-select').locator('.popup-select__option').filter({ hasText: 'Vertical' })
...
> 44  |   await expect(popup.locator('.popup-select__option', { hasText: 'Vertical' })).toBeVisible()
  ...
  116 | })
  117 | 
  118 | test('Alt+R depuis vertical → bascule en horizontal', async ({ page }) => {
  119 |   await gotoApp(page)
  120 |   await openSplit(page, 'Vertical')
  121 |   await expect(page.locator('body')).toHaveCSS('flex-direction', 'row')
  122 |   await press(page, 'Alt+r')
  123 |   await expect(page.locator('body')).toHaveCSS('flex-direction', 'column')
  124 | })
  125 | 
  126 | test('Alt+R depuis horizontal → bascule en vertical', async ({ page }) => {
  127 |   await gotoApp(page)
  128 |   await openSplit(page, 'Horizontal')
  129 |   await expect(page.locator('body')).toHaveCSS('flex-direction', 'column')
  130 |   await press(page, 'Alt+r')
  131 |   await expect(page.locator('body')).toHaveCSS('flex-direction', 'row')
  132 | })
  133 | 
```