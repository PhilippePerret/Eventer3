...
- Name: filter/panel-search.spec.js >> panneau persos : champ .panel-search visible après ':'
- Location: specs/e2e/filter/panel-search.spec.js:68:1
...
Locator: locator('#pane-1').contentFrame().locator('#persos-panel .filter-bar')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('#pane-1').contentFrame().locator('#persos-panel .filter-bar')

```

```yaml
- text: ✓ Cyrano 🫥 CY perso ✓ Roxane 🫥 RO perso Événement 1 — MON CY RO Événement 2 — DISP MODE NESTING
