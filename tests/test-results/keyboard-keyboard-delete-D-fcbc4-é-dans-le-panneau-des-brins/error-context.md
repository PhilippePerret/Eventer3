...
- Name: keyboard/keyboard-delete.spec.js >> Delete dans le panneau des brins >> Delete supprime le brin sélectionné dans le panneau des brins
- Location: specs/e2e/keyboard/keyboard-delete.spec.js:144:3
...
Locator: locator('#pane-1').contentFrame().locator('.brin-item .brin-item__title')
Expected substring: not "Autre brin"
Timeout: 5000ms
Error: element(s) not found

Call log:
  - Expect "not toContainText" with timeout 5000ms
  - waiting for locator('#pane-1').contentFrame().locator('.brin-item .brin-item__title')

```

```yaml
- text: "Mon brin MON brin #d9c8a9 Événement 1 — Événement 2 — DISP MODE NESTING"
...
```ts
  ...
  56  |     const items = pane1(page).locator('.project-item')
  57  |     const initialCount = await items.count()
  58  |     for (let i = 0; i < initialCount - 1; i++) {
  59  |       await press(page, 'Delete')
