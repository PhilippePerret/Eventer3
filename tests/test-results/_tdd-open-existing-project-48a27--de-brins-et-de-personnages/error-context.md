...
- Name: _tdd/open-existing-project.spec.js >> ouverture d'un projet existant : ses events affichent les marques de brins et de personnages
- Location: specs/e2e/_tdd/open-existing-project.spec.js:92:1
...
Call log:
  ...
  - waiting for locator('#pane-1').contentFrame().locator('#events-panel')
    13 × locator resolved to <div class="" id="events-panel">…</div>
       - unexpected value ""
...
> 98  |   await expect(pane1(page).locator('#events-panel')).toHaveClass(/event-list/)
  ...
  114 |   await pane1(page).locator('.ftpanel.kpanel').press('Enter')
