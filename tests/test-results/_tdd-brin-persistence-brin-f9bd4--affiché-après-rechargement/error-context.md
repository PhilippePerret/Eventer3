...
- Name: _tdd/brin-persistence.spec.js >> brin créé a bien un badge affiché après rechargement
- Location: specs/e2e/_tdd/brin-persistence.spec.js:50:1
...
Locator: locator('#pane-1').contentFrame().locator('.brin-item').nth(1).locator('[data-field="badge"]')
Expected: not ""
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('#pane-1').contentFrame().locator('.brin-item').nth(1).locator('[data-field="badge"]')

```

```yaml
- text: "Projet A --- roman Événement 1 — --- --- AUT Événement 2 — --- --- Mon brin #d9c8a9 MON brin Nouveau Brin #c8d9a9 --- ✓ Autre brin #a9d9c8 AUT brin"
...
```ts
  ...
  46 |   const badge = brinPersistant.locator('[data-field="badge"]')
...
> 72 |   await expect(newBrin.locator('[data-field="badge"]')).not.toHaveText('')
     |                                                             ^ Error: expect(locator).not.toHaveText(expected) failed
  73 | })
  74 | 
```