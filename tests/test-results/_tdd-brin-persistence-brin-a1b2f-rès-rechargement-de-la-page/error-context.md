...
Locator: locator('#pane-1').contentFrame().locator('.brin-item').filter({ hasText: 'Brin persistant' }).locator('[data-field="badge"]')
Expected: not ""
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('#pane-1').contentFrame().locator('.brin-item').filter({ hasText: 'Brin persistant' }).locator('[data-field="badge"]')

```

```yaml
- text: "Projet A --- roman Événement 1 — --- --- AUT Événement 2 — --- --- Mon brin #d9c8a9 MON brin Brin persistant #c8d9a9 --- ✓ Autre brin #a9d9c8 AUT brin"
