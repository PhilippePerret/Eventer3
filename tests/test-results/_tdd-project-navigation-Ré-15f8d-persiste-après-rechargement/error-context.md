...
- Name: _tdd/project-navigation.spec.js >> Régression : project_id propagé >> créer un brin → persiste après rechargement
- Location: specs/e2e/_tdd/project-navigation.spec.js:233:3
...
Locator: locator('#pane-1').contentFrame().locator('.brin-item input[name="title"]')
Expected: focused
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('#pane-1').contentFrame().locator('.brin-item input[name="title"]')

```

```yaml
- text: "Évènement un Intrigue principale IP #d9c8a9 AAA #888888 Projet A Évènement un — Évènement deux — Évènement trois — DISP MODE NESTING"
