...
- Name: filter/panel-search.spec.js >> panneau persos : taper 'cyr' cache 'Roxane'
- Location: specs/e2e/filter/panel-search.spec.js:76:1
...
Locator:  locator('#pane-1').contentFrame().locator('.perso-row')
Expected: 2
Received: 0
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('#pane-1').contentFrame().locator('.perso-row')
    ...
    - generic [ref=f1e2]:
      - generic [ref=f1e5]:
        - generic [active] [ref=f1e6]:
          - generic [ref=f1e8]: ✓
          - generic [ref=f1e9]:
            - generic [ref=f1e10]: Cyrano
            - generic [ref=f1e11]:
              - generic [ref=f1e12]: 🫥
              - generic [ref=f1e13]: CY
              - generic [ref=f1e14]: perso
        - generic [ref=f1e15]:
          - generic [ref=f1e17]: ✓
          - generic [ref=f1e18]:
            - generic [ref=f1e19]: Roxane
            - generic [ref=f1e20]:
              - generic [ref=f1e21]: 🫥
              - generic [ref=f1e22]: RO
              - generic [ref=f1e23]: perso
      - generic [ref=f1e26]:
        - generic [ref=f1e29]:
          - generic [ref=f1e30]: Événement 1
          - generic [ref=f1e32]: —
          - generic [ref=f1e33]:
            - generic [ref=f1e36]: MON
            - generic [ref=f1e38]:
              - generic [ref=f1e39]: CY
              - generic [ref=f1e40]: RO
        - generic [ref=f1e43]:
          - generic [ref=f1e44]: Événement 2
          - generic [ref=f1e46]: —
    - generic:
      - generic: DISP MODE NESTING
    - contentinfo "Raccourcis clavier" [ref=f1e47]
    - generic: AIDE ⇧⌘ ?
```
...
> 79  |   await expect(pane1(page).locator('.perso-row')).toHaveCount(2)
  ...
  120 | })
  121 | 
  122 | test("liste events : taper '1' cache 'Événement 2'", async ({ page }) => {
  123 |   await goToListerEvent(page, 'with-brins-and-persos')
  124 |   await revealFilter(page, '#events-panel')
  125 |   await expect(pane1(page).locator('.event-item')).toHaveCount(2)
  126 |   await pane1(page).locator('#events-panel .panel-search').fill('1')
  127 |   await expect(pane1(page).locator('.event-item:not(.hidden)')).toHaveCount(1)
  128 |   await expect(pane1(page).locator('.event-item.hidden')).toHaveCount(1)
  129 | })
  130 | 
  131 | test("liste events : vider le champ réaffiche tout", async ({ page }) => {
  132 |   await goToListerEvent(page, 'with-brins-and-persos')
  133 |   await revealFilter(page, '#events-panel')
  134 |   await pane1(page).locator('#events-panel .panel-search').fill('1')
  135 |   await expect(pane1(page).locator('.event-item:not(.hidden)')).toHaveCount(1)
  136 |   await pane1(page).locator('#events-panel .panel-search').fill('')
  137 |   await expect(pane1(page).locator('.event-item:not(.hidden)')).toHaveCount(2)
  138 | })
  139 | 
  140 | test("liste events : filtre remis à zéro quand on revient à la liste", async ({ page }) => {
  141 |   await goToListerEvent(page, 'with-brins-and-persos')
  142 |   await revealFilter(page, '#events-panel')
  143 |   await pane1(page).locator('#events-panel .panel-search').fill('1')
  144 |   await expect(pane1(page).locator('.event-item:not(.hidden)')).toHaveCount(1)
  145 |   await press(page, 'ArrowLeft') // retour projets
  146 |   await expect(pane1(page).locator('#projects-panel')).toBeVisible()
  147 |   await press(page, 'ArrowRight')
  148 |   await press(page, 'ArrowRight') // retour events
  149 |   await expect(pane1(page).locator('.event-item:not(.hidden)')).toHaveCount(2)
  150 |   const inputVal = await pane1(page).locator('#events-panel .panel-search').inputValue()
  151 |   expect(inputVal).toBe('')
  152 | })
  153 | 
```