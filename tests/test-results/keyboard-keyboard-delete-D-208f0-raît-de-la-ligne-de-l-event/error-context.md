...
- Name: keyboard/keyboard-delete.spec.js >> Delete dans le panneau des brins >> après suppression du brin coché, le badge disparaît de la ligne de l'event
- Location: specs/e2e/keyboard/keyboard-delete.spec.js:158:3
...
Locator: locator('#pane-1').contentFrame().locator('.event-item.selected').locator('.event-brins-badges .badge.brin')
Expected substring: "AUT"
Timeout: 5000ms
Error: element(s) not found

Call log:
  - Expect "toContainText" with timeout 5000ms
  - waiting for locator('#pane-1').contentFrame().locator('.event-item.selected').locator('.event-brins-badges .badge.brin')

```

```yaml
- text: "Mon brin MON brin #d9c8a9 ✓ Autre brin AUT brin #c8d9a9 Événement 1 — AUT Événement 2 — DISP MODE NESTING"
...
```ts
  ...
  63  |     await press(page, 'Delete')
  64  |     await expect(items).toHaveCount(1)
  65  |     await expect(pane1(page).locator('#notification')).toBeVisible()
  66  |   })
  67  | 
  68  | })
  69  | 
  ...
  72  | 
  73  | test.describe('Delete dans ListerEvent', () => {
  74  | 
  75  |   test.beforeEach(() => installFixtures('many-events'))
  76  | 
  77  |   test('Delete supprime l\'event sélectionné dans un ListerEvent', async ({ page }) => {
  78  |     await page.goto('/')
  79  |     await expect(pane1(page).locator('#projects-panel')).toBeVisible()
  80  |     await press(page, 'ArrowRight')
  81  |     await press(page, 'ArrowRight')
  82  |     await expect(pane1(page).locator('#events-panel')).toBeVisible()
  83  |     const items = pane1(page).locator('.event-item')
  84  |     const initialCount = await items.count()
  85  |     await press(page, 'Delete')
  86  |     await expect(items).toHaveCount(initialCount - 1)
  87  |   })
  88  | 
  89  |   test('la suppression de l\'event est persistante (rechargement)', async ({ page }) => {
  90  |     await page.goto('/')
