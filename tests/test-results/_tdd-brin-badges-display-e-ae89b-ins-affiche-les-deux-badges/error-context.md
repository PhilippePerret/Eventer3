...
- Name: _tdd/brin-badges-display.spec.js >> event avec deux brins affiche les deux badges
- Location: specs/e2e/_tdd/brin-badges-display.spec.js:30:1
...
Locator: locator('#pane-1').contentFrame().locator('.event-item').nth(2).locator('.event-brins-badges')
Expected substring: "AMO"
Timeout: 5000ms
Error: element(s) not found

Call log:
  - Expect "toContainText" with timeout 5000ms
  - waiting for locator('#pane-1').contentFrame().locator('.event-item').nth(2).locator('.event-brins-badges')

```

```yaml
- main: Scène du bal — --- --- Arrivée à Paris — --- --- La trahison — --- --- Retour au bal — --- ---
...
```ts
  ...
  15 | })
  16 | 
  17 | async function enterEventLister(page) {
  18 |   await page.goto('/')
  ...
  25 |   await enterEventLister(page)
  26 |   const e1 = pane1(page).locator('.event-item').nth(0)
  27 |   await expect(e1.locator('.event-brins-badges')).toContainText('AMO')
  28 | })
  29 | 
  30 | test('event avec deux brins affiche les deux badges', async ({ page }) => {
  31 |   await enterEventLister(page)
  32 |   const e3 = pane1(page).locator('.event-item').nth(2)
> 33 |   await expect(e3.locator('.event-brins-badges')).toContainText('AMO')
     |                                                   ^ Error: expect(locator).toContainText(expected) failed
  34 |   await expect(e3.locator('.event-brins-badges')).toContainText('INT')
  35 | })
  36 | 
  37 | test('event sans brin a badges vide', async ({ page }) => {
  38 |   await enterEventLister(page)
  39 |   const e4 = pane1(page).locator('.event-item').nth(3)
  40 |   await expect(e4.locator('.event-brins-badges')).toBeEmpty()
  41 | })
  42 | 
```