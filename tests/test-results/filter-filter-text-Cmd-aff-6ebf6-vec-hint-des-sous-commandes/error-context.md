...
- Name: filter/filter-text.spec.js >> Cmd+: affiche filter-bar avec hint des sous-commandes
- Location: specs/e2e/filter/filter-text.spec.js:25:1
...
Locator:  locator('#pane-1').contentFrame().locator('#filter-bar')
Expected: visible
Received: hidden
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('#pane-1').contentFrame().locator('#filter-bar')
    14 × locator resolved to <div class="hidden" id="filter-bar"></div>
