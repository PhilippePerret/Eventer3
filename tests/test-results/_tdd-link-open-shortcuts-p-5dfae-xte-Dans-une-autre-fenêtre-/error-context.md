...
- Name: _tdd/link-open-shortcuts.spec.js >> popup : troisième option contient badge 'a' et texte 'Dans une autre fenêtre'
- Location: specs/e2e/_tdd/link-open-shortcuts.spec.js:52:1
...
Locator: locator('#pane-1').contentFrame().locator('.floating-panel__item').nth(2).locator('.link-open-popup__key')
Expected: "a"
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('#pane-1').contentFrame().locator('.floating-panel__item').nth(2).locator('.link-open-popup__key')

```

```yaml
- text: Séquence 2 de Acte II Scène 1 de Séquence 2 de Acte II — Scène 2 de Séquence 2 de Acte II — Scène 3 de Séquence 2 de Acte II. Voir Séquence 3 de Acte I, Scène 1 de Séquence 4 de Acte III et Acte II — DISP MODE NESTING
...
> 56  |   await expect(third.locator('.link-open-popup__key')).toHaveText('a')
  ...
  112 |   await expect(pane1(page).locator('.notification')).toContainText(getErr(5200))
  113 | })
  114 | 
  115 | test("'c' sans cible sélectionnée → notification 'Aucune cible'", async ({ page }) => {
  116 |   await gotoEventList(page)
  117 |   await press(page, 'c')
  118 |   await expect(pane1(page).locator('.notification')).toBeVisible()
  119 |   await expect(pane1(page).locator('.notification')).toContainText(getErr(5200))
  120 | })
  121 | 
  122 | test("'a' sans cible sélectionnée → notification 'Aucune cible'", async ({ page }) => {
  123 |   await gotoEventList(page)
  124 |   await press(page, 'a')
  125 |   await expect(pane1(page).locator('.notification')).toBeVisible()
  126 |   await expect(pane1(page).locator('.notification')).toContainText(getErr(5200))
  127 | })
  128 | 
```