...
- Name: keyboard/keyboard-copy-cut-paste.spec.js >> ⌘+c + ⌘+v dans ProjectLister >> ⌘+c + ⌘+v : l'identifiant du projet collé est pX (pas null)
- Location: specs/e2e/keyboard/keyboard-copy-cut-paste.spec.js:225:3
...
Received string:  "2f25f634-37a7-42ef-9991-d2b57febedbf"
```
...
> 232 |     expect(idText.trim()).toMatch(/^p\d+$/)
  ...
  323 |     await expect(page.locator('#main-panel')).toHaveClass(/event-list/)
  324 |     const itemsA = page.locator('.event-item')
  325 |     const countABefore = await itemsA.count()
  326 |     const cutTitle = await page.locator('.event-item.selected').textContent()
  327 |     await page.keyboard.press('Meta+x')
  328 |     await expect(itemsA).toHaveCount(countABefore - 1)
  329 | 
  330 |     // Revenir à la liste des projets
  331 |     await page.keyboard.press('ArrowLeft')
  332 |     await expect(page.locator('#main-panel')).toHaveClass(/project-list/)
```