...
- Name: keyboard/keyboard-copy-cut-paste.spec.js >> ⌘+c dans ListerEvent >> après ⌘+c + ⌘+v, le collage est persistant
...
- iframe [active] [ref=e3]:
  - generic [ref=f5e1]:
    - generic [ref=f5e5]:
      - generic [ref=f5e8]:
        - generic [ref=f5e9]: Évènement un
        - generic [ref=f5e11]: —
      - generic [ref=f5e14]:
        - generic [ref=f5e15]: Évènement deux
        - generic [ref=f5e17]: —
      - generic [ref=f5e20]:
        - generic [ref=f5e21]: Évènement trois
        - generic [ref=f5e23]: —
    - generic:
      - generic: DISP MODE NESTING
    - contentinfo "Raccourcis clavier" [ref=f5e24]
    - generic: AIDE ⇧⌘ ?
```
...
> 65  |     await expect(items).toHaveCount(countBefore + 1)
  ...
  152 |     await pane1(page).locator('#brins-panel').waitFor()
  153 |     const items = pane1(page).locator('.brin-item')
  154 |     await press(page, 'Meta+x')
  155 |     await expect(items).toHaveCount(1)
  156 |     await press(page, 'Meta+x')
  157 |     await expect(items).toHaveCount(1)
  158 |     await expect(pane1(page).locator('#notification')).toBeVisible()
  159 |     await expect(pane1(page).locator('#notification')).toContainText('brin')
  160 |   })
  161 | 
  162 | })
  163 | 
  164 | test.describe('⌘+x interdit sur le dernier projet (ListerProject)', () => {
  165 | 
```