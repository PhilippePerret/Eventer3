...
class ProjectDbTest < Minitest::Test
  ...
  def setup
    ...
    @db_path = File.join(@data_dir, 'mon-projet', 'eventer.db')
  end
  ...
  def test_project_db_created_when_not_exists
    refute File.exist?(@db_path), "La DB projet ne devrait pas exister avant Bootstrap"

    Bootstrap.ensure_project_data!(@db_path)

    assert File.exist?(@db_path), "Bootstrap doit créer la DB projet au chemin fourni"
  end
  ...
  def test_open_project_opens_correct_file
    ...
    db = DB.open_project(@db_path)
    expected = File.realpath(@db_path)
