...
test('le sélecteur affiche les brins du projet', async ({ page }) => {
  ...
  await page.keyboard.press('Meta+:')
  await page.keyboard.press('b')
  await expect(page.locator('.filter-selector-row')).toHaveCount(2)
})
...
test('sélectionner b1 masque les events sans b1', async ({ page }) => {
  ...
  await page.keyboard.press('b')
  await expect(page.locator('#filter-selector-panel')).toBeVisible()
  await page.keyboard.press(' ')      // coche b1 (premier brin)
  await page.keyboard.press('Enter')  // applique
...
test('Escape dans le sélecteur n\'applique pas le filtre', async ({ page }) => {
  ...
  await page.keyboard.press('b')
  await expect(page.locator('#filter-selector-panel')).toBeVisible()
  await page.keyboard.press(' ')      // coche b1
