...
async function setupTargetsAndOpenPanel(page, count = 1) {
  ...
  for (let i = 0; i < count; i++) {
    if (i > 0) await pane1(page).locator('#main-panel').press('ArrowDown')
...
test('overflow : item pinned non supprimé quand pile déborde', async ({ page }) => {
  ...
  await setupTargetsAndOpenPanel(page)
  await pane1(page).locator('#main-panel').press('Meta+ArrowUp')          // pin le premier item
  await pane1(page).locator('.notification').waitFor({ state: 'hidden' })
  await pane1(page).locator('#main-panel').press('Escape')   // ferme TargetsPanel → retour item-edition
  await pane1(page).locator('#main-panel').press('Escape')   // annule édition → mode normal
