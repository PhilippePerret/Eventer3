...
test('n ouvre directement le FilePicker sans demander le titre', async ({ page }) => {
  ...
  await expect(page.locator('.project-item.selected input[name="title"]')).not.toBeVisible()
})

test('FilePicker liste les entrées du dossier courant', async ({ page }) => {
  await openPickerViaN(page)
  await expect(page.locator('.file-picker__entry')).toHaveCount(4)
})
...
test('entrées triées alphabétiquement (fichiers et dossiers mélangés)', async ({ page }) => {
  ...
  const first = page.locator('.file-picker__entry').first()
...
test('FilePicker sélectionne la première entrée par défaut', async ({ page }) => {
  await openPickerViaN(page)
  await expect(page.locator('.file-picker__entry.selected')).toHaveCount(1)
  await expect(page.locator('.file-picker__entry').first()).toHaveClass(/selected/)
})
...
test('↓ déplace la sélection vers le bas', async ({ page }) => {
  ...
  await expect(page.locator('.file-picker__entry').nth(1)).toHaveClass(/selected/)
})
...
test('→ ne fait rien sur un fichier', async ({ page }) => {
  ...
  await expect(page.locator('.file-picker__entry.selected')).toHaveAttribute('data-type', 'file')
  await page.keyboard.press('ArrowRight')
  // Path ne change pas
  await waitForPath(page, TEST_DIR)
})
...
test('n dans le picker affiche un champ de saisie pour nouveau dossier', async ({ page }) => {
  await openPickerViaN(page)
  await page.keyboard.press('n')
  await expect(page.locator('.file-picker__new-folder-input')).toBeVisible()
})
...
test('n + nom + Enter crée le dossier et l\'ajoute à la liste', async ({ page }) => {
  ...
  await waitForPath(page, path.join(TEST_DIR, 'Roman-Alpha'))
  await page.keyboard.press('n')
  await page.locator('.file-picker__new-folder-input').fill('Mon-Nouveau-Dossier')
  await page.keyboard.press('Enter')
  await expect(page.locator('.file-picker__entry-name').filter({ hasText: 'Mon-Nouveau-Dossier' })).toBeVisible()
...
test('Escape pendant création dossier annule sans créer', async ({ page }) => {
  ...
  await waitForPath(page, path.join(TEST_DIR, 'Roman-Alpha'))
  await page.keyboard.press('n')
  await page.locator('.file-picker__new-folder-input').fill('Dossier-Annule')
  await page.keyboard.press('Escape')
...
test('Tab affiche le menu d\'arborescence des dossiers parents', async ({ page }) => {
  ...
  await expect(page.locator('.file-picker__path-menu')).toBeVisible()
})
...
test('le menu arborescence contient le dossier courant et ses parents', async ({ page }) => {
  ...
  await waitForPath(page, path.join(TEST_DIR, 'Roman-Alpha'))
  await page.keyboard.press('Tab')
  const items = page.locator('.file-picker__path-menu-item')
...
test('Footer affiche ↩︎ (droite) et ␛ (gauche)', async ({ page }) => {
  await openPickerViaN(page)
  await expect(page.locator('.file-picker__select-btn')).toContainText('↩︎')
  await expect(page.locator('.file-picker__cancel-key')).toContainText('␛')
})
...
test('bouton sélection désactivé sur fichier, actif sur dossier', async ({ page }) => {
  ...
  await expect(page.locator('.file-picker__entry.selected')).toHaveAttribute('data-type', 'file')
  await expect(page.locator('.file-picker__select-btn')).toHaveClass(/disabled/)
  // Naviguer sur Roman-Alpha (dossier) → bouton actif
  await page.keyboard.press('ArrowDown')
  await expect(page.locator('.file-picker__entry.selected')).toHaveAttribute('data-type', 'directory')
  await expect(page.locator('.file-picker__select-btn')).not.toHaveClass(/disabled/)
})
