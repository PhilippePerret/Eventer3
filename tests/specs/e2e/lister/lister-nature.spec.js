...
async function applyNaturePanel(page) {
  await pane1(page).locator('body').press('Tab')   // footer focus → Annuler
  await pane1(page).locator('body').press('Tab')   // footer focus → Appliquer
  await pane1(page).locator('body').press('Enter') // appliquer
}
...
async function setRomanMan(page) {
  ...
  await pane1(page).locator('body').press('Enter')                          // ouvre popup projet
  await expect(pane1(page).locator('.popup-select')).toBeVisible()
  await pane1(page).locator('body').press('ArrowUp')                        // film/BD
  await pane1(page).locator('body').press('ArrowUp')                        // roman
  await pane1(page).locator('body').press('Enter')                          // roman
  await pane1(page).locator('body').press('ArrowDown')                      // champ évènemencier
  await pane1(page).locator('body').press('Enter')                          // ouvre popup évènemencier
  await expect(pane1(page).locator('.popup-select')).toBeVisible()
  await pane1(page).locator('body').press('ArrowUp')                        // manuscrit
  await pane1(page).locator('body').press('Enter')                          // manuscrit
  await applyNaturePanel(page)
  await expect(pane1(page).locator('.confirm-dialog')).toBeVisible()
  await pane1(page).locator('body').press('Escape')                         // refuser man_depth
...
test("projet 'film' → popup évènemencier propose 'scénario'", async ({ page }) => {
  ...
  await pane1(page).locator('body').press('t')
  await pane1(page).locator('body').press('Enter')
  await pane1(page).locator('body').press('ArrowUp')    // film/BD (index 1 depuis —)
  await pane1(page).locator('body').press('Enter')
  await pane1(page).locator('body').press('ArrowDown')  // champ évènemencier
  await pane1(page).locator('body').press('Enter')
...
test("film+scénario → #main-panel a la classe 'film-man'", async ({ page }) => {
  ...
  await pane1(page).locator('body').press('ArrowDown') // champ évènemencier
  await pane1(page).locator('body').press('Enter')
  await pane1(page).locator('body').press('ArrowUp')   // scénario (index 0 depuis évènemencier)
  await pane1(page).locator('body').press('Enter')
  await applyNaturePanel(page)
  await expect(pane1(page).locator('.confirm-dialog')).toBeVisible()
  await pane1(page).locator('body').press('Escape')    // refuser man_depth
...
test("confirmer 'o' → man_depth sauvegardé, sibling lister devient roman-man", async ({ page }) => {
  ...
  await applyNaturePanel(page)
  await expect(pane1(page).locator('.confirm-dialog')).toBeVisible()
  await pane1(page).locator('body').press('Enter') // confirmer man_depth (oui)
  await expect(pane1(page).locator('.confirm-dialog')).not.toBeVisible()
  await pane1(page).locator('body').press('ArrowLeft')
  await pane1(page).locator('body').press('ArrowDown')
  await pane1(page).locator('body').press('ArrowRight')
...
test("depth = man_depth → appliquer ferme sans confirmation", async ({ page }) => {
  ...
  await applyNaturePanel(page)
  await expect(pane1(page).locator('.confirm-dialog')).toBeVisible()
  await pane1(page).locator('body').press('Enter') // man_depth = 2 (oui)
  await expect(pane1(page).locator('.confirm-dialog')).not.toBeVisible()
...
test("nature null à man_depth → panneau affiche 'manuscrit', popup focused sur 'défaut'", async ({ page }) => {
  ...
  await applyNaturePanel(page)
  await expect(pane1(page).locator('.confirm-dialog')).toBeVisible()
  await pane1(page).locator('body').press('Enter')     // oui → man_depth=2
  await expect(pane1(page).locator('.confirm-dialog')).not.toBeVisible()
...
test("Tab×3 ramène à aucun footer sélectionné → Enter ouvre le popup", async ({ page }) => {
  ...
  await pane1(page).locator('body').press('Tab')   // → Annuler
  await pane1(page).locator('body').press('Tab')   // → Appliquer
  await pane1(page).locator('body').press('Tab')   // → aucun (retour à -1)
