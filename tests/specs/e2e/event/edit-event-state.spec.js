...
test("Enter sélectionne l'option et ferme le popup", async ({ page }) => {
  ...
  await pane1(page).locator('#main-panel').press('ArrowDown')  // ébauche
  await pane1(page).locator('#main-panel').press('Enter')
...
test("valider l'édition commit le titre et l'état", async ({ page }) => {
  ...
  await pane1(page).locator('#main-panel').press('Enter')  // commit
...
test("Escape en édition restaure l'état original même si popup a été utilisé", async ({ page }) => {
  ...
  await pane1(page).locator('#main-panel').press('ArrowDown')
  await pane1(page).locator('#main-panel').press('ArrowDown')  // ébauche
  await pane1(page).locator('#main-panel').press('Enter')      // sélectionne ébauche
...
test("choisir un état en édition l'enregistre (persistance après rechargement)", async ({ page }) => {
  ...
  await pane1(page).locator('#main-panel').press('ArrowDown')  // ébauche
  await pane1(page).locator('#main-panel').press('Enter')
  await pane1(page).locator('#main-panel').press('Enter')  // confirme l'édition
