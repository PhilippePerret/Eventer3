...
test('go vers item racine (e3=Acte III) depuis sous-lister', async ({ page }) => {
  ...
  await pane1(page).locator('.event-item.selected').press('ArrowDown') // → e5 (Séq 2 Acte I, lien vers e3)
  await pane1(page).locator('.event-item.selected').press('Tab')    // active lien "Acte III"
...
test('go vers item depth=1 (e6=Séq 3 Acte I) depuis sous-lister différent', async ({ page }) => {
  await gotoEventList(page)
  await pane1(page).locator('.event-item.selected').press('ArrowDown') // → e2 (Acte II)
  await enterSubLister(page)             // entre dans Acte II
  await pane1(page).locator('.event-item.selected').press('ArrowDown') // → e9 (Séq 2 Acte II)
  await enterSubLister(page)             // entre dans Séq 2 Acte II
  await pane1(page).locator('.event-item.selected').press('ArrowDown')
  await pane1(page).locator('.event-item.selected').press('ArrowDown') // → e33 (Scène 3, 3 liens)
  await pane1(page).locator('.event-item.selected').press('Tab')       // lien 1 : e6 "Séquence 3 de Acte I"
  await openLinkPopupAndGo(page)
...
test('go vers item depth=2 (e49=Scène 1 Séq 4 Acte III) depuis sous-lister différent', async ({ page }) => {
  ...
  await enterSubLister(page)
  await pane1(page).locator('.event-item.selected').press('ArrowDown')
  await pane1(page).locator('.event-item.selected').press('ArrowDown') // → e33
  await pane1(page).locator('.event-item.selected').press('Tab')       // lien 1 : e6
  await pane1(page).locator('.event-item.selected').press('Tab')       // lien 2 : e49 "Scène 1 de Séquence 4 de Acte III"
  await openLinkPopupAndGo(page)
