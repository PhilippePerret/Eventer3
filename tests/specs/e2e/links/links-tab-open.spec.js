...
test('TAB sur item avec lien → premier lien activé', async ({ page }) => {
  ...
  await pane1(page).locator('#main-panel').press('ArrowDown') // → s2a1
  await pane1(page).locator('#main-panel').press('Tab')
...
test('TAB cycle : 2e TAB → 2e lien activé', async ({ page }) => {
  await gotoEventList(page)
  await pane1(page).locator('#main-panel').press('ArrowDown') // → a2
  await enterSubLister(page)             // entre dans Acte II
  await pane1(page).locator('#main-panel').press('ArrowDown') // → s2a2
  await enterSubLister(page)             // entre dans Séquence 2
  await pane1(page).locator('#main-panel').press('ArrowDown')
  await pane1(page).locator('#main-panel').press('ArrowDown') // → sc3s2a2 (3e scène, 2 liens)
  await pane1(page).locator('#main-panel').press('Tab') // lien 1
  await pane1(page).locator('#main-panel').press('Tab') // lien 2
...
test('TAB bouclage : après dernier lien → retour au premier', async ({ page }) => {
  ...
  await enterSubLister(page)
  await pane1(page).locator('#main-panel').press('ArrowDown')
  await pane1(page).locator('#main-panel').press('ArrowDown') // → sc3s2a2 (3 liens)
  await pane1(page).locator('#main-panel').press('Tab') // lien 1
  await pane1(page).locator('#main-panel').press('Tab') // lien 2
  await pane1(page).locator('#main-panel').press('Tab') // lien 3
  await pane1(page).locator('#main-panel').press('Tab') // bouclage → lien 1
...
test('changer item sélectionné efface le lien actif', async ({ page }) => {
  ...
  await enterSubLister(page)
  await pane1(page).locator('#main-panel').press('ArrowDown')
  await pane1(page).locator('#main-panel').press('ArrowDown') // → sc3s2a2
  await pane1(page).locator('#main-panel').press('Tab')
...
test('MAJ+TAB cycle à l\'envers : TAB → lien 1, Shift+TAB → lien 3 (wrap arrière)', async ({ page }) => {
  ...
  await pane1(page).locator('#main-panel').press('ArrowDown')
  await pane1(page).locator('#main-panel').press('ArrowDown') // → sc3s2a2 (3 liens)
  await pane1(page).locator('#main-panel').press('Tab')       // lien 1 : "Séquence 3 de Acte I"
  // Shift+TAB depuis index 0 → backward wrap → index 2 (lien 3 : "Acte II")
  await pane1(page).locator('#main-panel').press('Shift+Tab')
...
test("'o' sans lien actif sur item sans liens → notification", async ({ page }) => {
  await gotoEventList(page)
  await pane1(page).locator('#main-panel').press('o')
  await expect(pane1(page).locator('.notification')).toBeVisible()
  await expect(pane1(page).locator('.notification')).toContainText('aucun lien')
})
