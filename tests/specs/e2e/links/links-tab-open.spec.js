...
test('TAB sur item avec lien → premier lien activé', async ({ page }) => {
  ...
  await pane1(page).locator('.event-item.selected').press('ArrowDown') // → s2a1
  await pane1(page).locator('.event-item.selected').press('Tab')
...
test('TAB cycle : 2e TAB → 2e lien activé', async ({ page }) => {
  ...
  await pane1(page).locator('.event-item.selected').press('ArrowDown') // → sc3s2a2 (3e scène, 2 liens)
  await pane1(page).locator('.event-item.selected').press('Tab') // lien 1
  await pane1(page).locator('.event-item.selected').press('Tab') // lien 2
...
test('TAB bouclage : après dernier lien → retour au premier', async ({ page }) => {
  ...
  await enterSubLister(page)
  await pane1(page).locator('.event-item.selected').press('ArrowDown')
  await pane1(page).locator('.event-item.selected').press('ArrowDown') // → sc3s2a2 (3 liens)
  await pane1(page).locator('.event-item.selected').press('Tab') // lien 1
  await pane1(page).locator('.event-item.selected').press('Tab') // lien 2
  await pane1(page).locator('.event-item.selected').press('Tab') // lien 3
  await pane1(page).locator('.event-item.selected').press('Tab') // bouclage → lien 1
...
test('MAJ+TAB cycle à l\'envers : TAB → lien 1, Shift+TAB → lien 3 (wrap arrière)', async ({ page }) => {
  ...
  await pane1(page).locator('.event-item.selected').press('ArrowDown')
  await pane1(page).locator('.event-item.selected').press('ArrowDown') // → sc3s2a2 (3 liens)
  await pane1(page).locator('.event-item.selected').press('Tab')       // lien 1 : "Séquence 3 de Acte I"
