...
test("lien cassé : 'o' → notification, pas de popup", async ({ page }) => {
  ...
  await pane1(page).locator('.event-item.selected').press('Tab')   // active [Acte III](e3) — valide
  await pane1(page).locator('.event-item.selected').press('Tab')   // active [Fantôme](e999) — cassé
...
test("lien cassé : 'g' → notification sans naviguer", async ({ page }) => {
  ...
  await pane1(page).locator('.event-item.selected').press('Tab')   // active [Fantôme](e999)
