...
> ⚠️ **Avertissement** : ce fichier décrit l'ancienne architecture. Une refactorisation majeure est en cours (session 2026-06-17). En cas de conflit avec `doc/dev/Architecture-cible.md`, c'est `Architecture-cible.md` qui fait foi.
...
- Le `type` d’un `Item` (`Project`, `Event`, `Brin`, `Perso`, `Script`) n’est JAMAIS la classe spécialisée minorisée de l’item. Le type de `Project` n’est JAMAIS `perso`, le `type` d’un `Event` n’est JAMAIS `event`, le `type` d’un `Perso` n’est JAMAIS `perso`. Voir les [types possibles](../doc/dev/Specs-modeles.md#item-types) dans le document des modèles.
